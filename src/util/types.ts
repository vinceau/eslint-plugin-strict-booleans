 ...
 */
export function isTypeArrayTypeOrUnionOfArrayTypes(type: ts.Type, checker: ts.TypeChecker): boolean {
  for (const t of unionTypeParts(type)) {
    if (!(checker as any).isArrayType(t)) {
      return false;
    }
  }
 ...
 */
export function containsAllTypesByName(type: ts.Type, allowAny: boolean, allowedNames: Set<string>): boolean {
 ...
 */
export function getTypeName(typeChecker: ts.TypeChecker, type: ts.Type): string {
...
export function isNullableType(
  type: ts.Type,
  { isReceiver = false, allowUndefined = true }: { isReceiver?: boolean; allowUndefined?: boolean } = {},
): boolean {
 ...
 */
export function getDeclaration(checker: ts.TypeChecker, node: ts.Expression): ts.Declaration | null {
 ...
 * @param isReceiver true if the type is a receiving type (i.e. the type of a called function's parameter)
 */
export function isTypeFlagSet(type: ts.Type, flagsToCheck: ts.TypeFlags, isReceiver?: boolean): boolean {
  const flags = getTypeFlags(type);
 ...
 */
export function typeIsOrHasBaseType(type: ts.Type, parentType: ts.Type): boolean {
...
export function getTypeArguments(type: ts.TypeReference, checker: ts.TypeChecker): readonly ts.Type[] {
 ...
 * @returns true if the type is `any[]`
 */
export function isTypeAnyArrayType(type: ts.Type, checker: ts.TypeChecker): boolean {
  return (
 ...
 * @returns true if the type is `unknown[]`
 */
export function isTypeUnknownArrayType(type: ts.Type, checker: ts.TypeChecker): boolean {
  return (
    checker.isArrayType(type) &&
    isTypeUnknownType(
      // getTypeArguments was only added in TS3.7
 ...
 */
export function isAnyOrAnyArrayTypeDiscriminated(node: ts.Node, checker: ts.TypeChecker): AnyType {
 ...
 */
export function getContextualType(checker: ts.TypeChecker, node: ts.Expression): ts.Type | undefined {
  ...
  } else if (isVariableDeclaration(parent) || isPropertyDeclaration(parent) || isParameterDeclaration(parent)) {
